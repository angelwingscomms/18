...
{#if v.show_binaural_settings}
	<div class="overlay" role="presentation" onclick={() => (v.show_binaural_settings = false)}>
		<div
			...
			onkeydown={(e) => e.key === 'Escape' && (v.show_binaural_settings = false)}
		>
			...
			<div class="modal-body">
					...
					>
						<span>
							<span class="select-value"
								>{model_options.find((o) => o.v === v.model)?.l ?? v.model}</span
							>
				...
				<section class="setting-section">
					...
					>
						<span>
							<span class="select-value"
								>{voice_options.find((o) => o.v === v.voice_name)?.l ?? v.voice_name}</span
							>
							<span class="select-desc"
								>{voice_options.find((o) => o.v === v.voice_name)?.d ?? ''}</span
							>
						</span>
						<span class="select-arrow">⌄</span>
					</button>
						...
						<div class="voice-list" role="listbox">
							{#each voice_options as option (option.v)}
								<button
									...
									onclick={() => {
										v.voice_name = option.v;
										v.show_voice_menu = false;
									}}
								>
									<span class="option-name">{option.l}</span>
									<span class="option-desc">{option.d}</span>
					...
					<label class="toggle-row">
						<span>
							...
							<span class="toggle-desc"
								>Press Tab to insert a tab in the note instead of leaving it.</span
							>
						</span>
				...
				<div class="toggle-section">
					<label class="toggle-row">
						<span>
							<span class="toggle-label">Fold lines</span>
							<span class="toggle-desc"
								>Collapse tab-indented blocks in the note with a gutter toggle.</span
							>
						</span>
						<input type="checkbox" bind:checked={v.fold_lines} class="toggle-input" />
						<span class="toggle-switch {v.fold_lines ? 'on' : ''}"></span>
					</label>
				</div>
				...
				<section class="setting-section">
					...
					<span class="toggle-desc" style="margin-top:-4px"
						>Appended to the base system prompt sent to Gemini.</span
					>
					<textarea
						class="prompt-input"
				...
				<section class="setting-section">
					...
					<span class="toggle-desc" style="margin-top:-4px"
						>432 Hz left ear / 439 Hz right ear (7 Hz delta)</span
					>
			...
			<div class="modal-footer">
				<button class="btn-secondary" onclick={() => (v.show_binaural_settings = false)}
					>Close</button
				>
			</div>
		</div>
